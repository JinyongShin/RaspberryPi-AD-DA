...
file = open("/home/pi/data_log_test3.csv", "a")
if os.stat("/home/pi/data_log_test3.csv").st_size == 0:
...
# when "used" used, make sure to include range() with the amount of data points requested
    #while True:
    for x in range(100):
        ADC_Value = ADC.ADS1256_GetAll()
        file.write(str(x)+","+str(datetime.datetime.now())+","+str(ADC_Value[0]*5.0/0x7fffff)+","+str(ADC_Value[1]*5.0/0x7fffff)+"\n")
        file.flush()
        sleep(0.5) #recording data every 0.5 seconds
